...
fn_t_score <- gt::md("CAARS _T_-score: Mean = 50 [50th‰], SD ± 10 [16th‰, 84th‰].
...
grp_adhd <- list(
  ...
  t_score = c("CAARS Self-Report", "CAARS Observer-Report", "CAARS-2 Self-Report", "CAARS-2 Observer-Report")
)
grp_caars <- list(
  t_score = c("CAARS Self-Report", "CAARS Observer-Report", "CAARS-2 Self-Report", "CAARS-2 Observer-Report")
)
...
# variables
data_adhd_self <-
  dplyr::mutate(data_adhd_self, scale = stringr::str_remove_all(scale, "CAARS-SR |CEFI-SR |CAARS-2 SR "))

fn_standard_score <- fn_standard_score
fn_t_score <- fn_t_score
grp_self <- list(
  standard_score = c("CEFI Self-Report"),
  t_score = c("CAARS Self-Report", "CAARS-2 Self-Report")
)
...
# variables
data_adhd_observer <-
  dplyr::mutate(data_adhd_observer, scale = stringr::str_remove_all(scale, "CAARS-SR |CEFI-SR |CAARS-2 SR "))

fn_standard_score <- fn_standard_score
fn_t_score <- fn_t_score
grp_observer <- list(
  standard_score = c("CEFI Observer-Report"),
  t_score = c("CAARS Observer-Report", "CAARS-2 Observer-Report")
)

# variables
data_adhd_observer <-
  dplyr::mutate(data_adhd_observer, scale = stringr::str_remove_all(scale, "CAARS-OR |CEFI-OR |CAARS-2 OR "))
pheno <- "adhd_observer"
