...
```{r}
plucked_tables_caars2 <- bwu::gpluck_extract_tables(
  file = file,
  pages = pages,
  area = area,
  guess = NULL,
...
df1 <- as.data.frame(plucked_tables_caars2[[1]])
...
```{r}
# Convert matrix to data frame and set proper column names
df2 <- as.data.frame(plucked_tables_caars2[[2]])
...
```{r pluck3}
# Convert matrix to data frame and set proper column names
df3 <- as.data.frame(plucked_tables_caars2[[3]])
  ...
  dplyr::mutate(
    description = dplyr::case_when(
      ...
      scale == "Emotional Dysregulation" ~
        "i.e., has a hard time calming down, has trouble hiding their emotions",
      ...
      scale == "CAARS 2-ADHD Index" ~
        "i.e., a composite indicator for identifying individuals 'at-risk' for ADHD",
...
if (params$test == "caars2_sr") {
    ...
    dplyr::mutate(
      result = dplyr::case_when(
        scale == "Inattention/Executive Dysfunction" ~ glue::glue(
          "- Self-reported {scale} ({description}) was {range}.\n\n"
        ),
        ...
        scale == "CAARS 2-ADHD Index" ~ glue::glue(
          "- Self-reported {scale} ({description}) was {probability_score}.\n\n"
        ),
  ...
  table <-
    ...
    dplyr::mutate(
      result = dplyr::case_when(
        scale == "Inattention/Executive Dysfunction" ~ glue::glue(
          "- Observer-reported {scale} ({description}) was {range}.\n\n"
        ),
        scale == "Hyperactivity" ~ glue::glue(
          "- Observer-reported {scale} ({description}) was {range}.\n\n"
        ),
        ...
        scale == "Negative Self-Concept" ~ glue::glue(
          "- Observer-reported {scale} ({description}) was {range}.\n\n"
        ),
        ...
        scale == "CAARS 2-ADHD Index" ~ glue::glue(
          "- Observer-reported {scale} ({description}) was {probability_score}.\n\n"
        ),
        TRUE ~ as.character(result)
      )
    )
