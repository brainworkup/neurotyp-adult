...
readr::write_csv(iq, paste0(pheno, ".csv"), na = "", col_names = TRUE, append = FALSE)
```
...
# remove <- c("Neuropsychological Function", "Working Memory", "Processing Speed")
# data_iq <- dplyr::filter(data_iq, !subdomain %in% remove)
keep <- c("General Ability", "Crystallized Knowledge", "Fluid Reasoning")
data_iq <- dplyr::filter(data_iq, subdomain %in% keep)
