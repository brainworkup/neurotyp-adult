...
```{r}
#| label: data-adhd
data <- bwu::read_data(pheno = params$pheno)
```
...
# ADHD
domains <- c("ADHD")
data_adhd <- bwu::filter_domain_scale(data,
  domain = domains,
  scale = scales
)
# EF
...
# Flatten the text
bwu::flatten_scale_text(data = data_sr, file = "_10_adhd_raw.qmd")
```
...
# Flatten the text
bwu::flatten_scale_text(data = data_or, file = "_10_adhd_raw.qmd")
```
...
source_note <- gt::md("*Note:* CAARS: Standard scores (Mean = 50, SD = 10), higher scores reflect reduced functioning.
...
#let file_fig = "fig_cefi.png"
#domain(
  title: "ADHD/Executive Functioning Test Scores",
  file_qtbl,
  file_fig
  )
```
